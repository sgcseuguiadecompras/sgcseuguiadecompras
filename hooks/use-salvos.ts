...
export function useSalvos() {
  ...
  const carregarSalvos = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/salvos?usuario_id=${usuarioId}`)
      if (res.ok) {
  ...
  const removerSalvo = useCallback(async (produtoId: string) => {
    ...
    try {
      const res = await fetch(`/api/salvos?usuario_id=${usuarioId}&produto_id=${produtoId}`, {
        method: "DELETE",
      })
